...
def get_word_rows(stream=sys.stdin):
    for line in stream:
        path, word, count = line.split('\t')
        count = int(count)
        yield path,word,count

def get_cluster_rows(stream=sys.stdin):
    for path, rows in itertools.groupby(get_word_rows(stream), key=lambda x: x[0]):
...
if __name__=='__main__':
    for path, nwords, wordcounts, allwc in get_cluster_rows():
        ...
        wc1 = ' '.join("<span class=w>{w}</span>".format(
            w=htmlescape(w)) for w,c in wordcounts)
        ...
        with open("paths/{path}.html".format(**locals()),'w') as f:
